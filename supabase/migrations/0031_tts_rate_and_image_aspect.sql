alter table public.tasks add column if not exists tts_rate text not null default '-8%';
...
begin
  foreach k in array kinds loop
    ...
    if k = 'book' then initial_status := 'done'; stage_params := jsonb_build_object('skipped', true, 'reason', 'book metadata is sourced from rewrite'); end if;
