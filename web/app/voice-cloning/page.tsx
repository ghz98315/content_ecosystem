...
type VoiceProfile = { id: string; display_name: string; provider: "edge" | "cosyvoice2"; voice_id: string; enabled: boolean; authorization_confirmed: boolean; sample_path?: string | null; updated_at?: string };
...
export default function VoiceCloningPage() {
  ...
  const [provider, setProvider] = useState<"edge" | "cosyvoice2">("edge");
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      const { data: profile, error: insertError } = await supabase.from("voice_profiles").insert({ owner: userData.user.id, display_name: name.trim(), provider, voice_id: voiceId.trim(), sample_path: result.path, authorization_confirmed: authorized }).select().single();
